...
public class PhotoAlbumHelper {
	...
	private static PhotoAlbumHelper instance;
	...
	void buildImagesBucketList() {
		...
		if (cur.moveToFirst()) {
			...
			do {
				...
				if (bucket == null) {
					...
					bucket.imageList = new ArrayList<Photo>();
		...
		Iterator<Entry<String, PhotoAlbum>> itr = bucketList.entrySet()
				.iterator();
		while (itr.hasNext()) {
			Map.Entry<String, PhotoAlbum> entry = (Map.Entry<String, PhotoAlbum>) itr
					.next();
			PhotoAlbum bucket = entry.getValue();
			Log.d(TAG, entry.getKey() + ", " + bucket.bucketName + ", "
					+ bucket.count + " ---------- ");
			for (int i = 0; i < bucket.imageList.size(); ++i) {
				Photo image = bucket.imageList.get(i);
